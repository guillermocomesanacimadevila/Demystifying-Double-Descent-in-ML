...
np.random.seed(42)
num_snps = 1000
beta_ldl = np.random.normal(0, 1, num_snps)  
true_gamma = 0.5
beta_chd = true_gamma * beta_ldl + np.random.normal(0, 0.2, num_snps)  
data = pd.DataFrame({"Beta_LDL_c": beta_ldl, "Beta_CHD_risk": beta_chd})
data.to_csv("GWAS_SNP_Effect_Simulation.csv", index=False)

snps_range = np.arange(50, num_snps - 50, 50)  
train_errors, test_errors = [], []
for num_features in snps_range:
    train_size = int(0.8 * num_features)  
