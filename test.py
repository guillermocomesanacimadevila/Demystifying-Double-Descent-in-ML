...
# Simulated dataset similar to the paper
np.random.seed(42)
n_samples = 1000
n_features = 50
...
# Phase 1: Increasing Boosting Rounds (Fixed Ensemble Size = 1)
for rounds in boosting_rounds:
    gb = GradientBoostingRegressor(n_estimators=rounds, max_depth=3, learning_rate=0.1,
...
# Phase 2: Increasing Ensemble Size (Fixed Boosting Rounds = 50)
...
# Interpolation threshold (peak before second descent)
...
# Panel 3: Composite Axis
axes[2].plot(range(len(composite_test_errors)), composite_test_errors, color='black', linewidth=2)
axes[2].axvline(interpolation_idx, linestyle='--', color='red', label='Interpolation Threshold')
...
axes[2].set_xticklabels([composite_x_labels[i] for i in range(0, len(composite_x_labels), max(len(composite_x_labels)//10, 1))],
